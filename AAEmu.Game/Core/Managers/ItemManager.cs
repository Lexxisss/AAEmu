...
using NLog;
using static AAEmu.Game.Core.Managers.ItemManager;
...
public class ItemManager : Singleton<ItemManager>
{
    ...
    private readonly Dictionary<int, Dictionary<int, GradeEnchantRatio>> _gradeEnchantRatios
    = new();
    // group_id -> (grade -> ratio

    // item_id -> group_id
    private readonly Dictionary<int, int> _enchantItemGroups = new();

    private const int DefaultEnchantGroupId = 9000001;
    // ItemSlaveEquip
    ...
    public class GradeEnchantRatio
    {
    ...
    private readonly Dictionary<int, List<GradeEnchantRatio>> _gradeEnchantRatiosByGroup = new();
    ...
    public void Load()
    {
        ...
        */
        ...
        {
            LoadGradeEnchantRatios(connection);
    ...
    private void LoadGradeEnchantRatios(SqliteConnection connection)
    {
        _gradeEnchantRatios.Clear();
        _enchantItemGroups.Clear();

        // item_enchant_ratios (group + grade)
        using (var command = connection.CreateCommand())
        {
            ...
            using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
            {
                while (reader.Read())
                {
                    int groupId = reader.GetInt32("item_enchant_ratio_group_id");
                    int grade = reader.GetInt32("grade");

                    var ratio = new GradeEnchantRatio
                    {
                        ImplId = groupId,
                        Grade = grade,
                        SuccessRatio = reader.GetInt32("grade_enchant_success_ratio"),
                        GreatSuccessRatio = reader.GetInt32("grade_enchant_great_success_ratio"),
                        BreakRatio = reader.GetInt32("grade_enchant_break_ratio"),
                        DowngradeRatio = reader.GetInt32("grade_enchant_downgrade_ratio"),
                        Cost = reader.GetInt32("grade_enchant_cost"),
                        DowngradeMin = reader.GetInt32("grade_enchant_downgrade_min"),
                        DowngradeMax = reader.GetInt32("grade_enchant_downgrade_max"),
                        CurrencyId = reader.GetInt32("currency_id")
                    };

                    if (!_gradeEnchantRatios.TryGetValue(groupId, out var grades))
                    {
                        grades = new Dictionary<int, GradeEnchantRatio>();
                        _gradeEnchantRatios[groupId] = grades;
                    }
        ...
        // item_enchant_ratio_items (item → group)
        using (var command = connection.CreateCommand())
        {
            ...
            using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
            {
                while (reader.Read())
                {
                    ...
                    _enchantItemGroups[itemId] = groupId;
                }
            }
        }

        Logger.Info($"[Enchant Load] Groups: {_gradeEnchantRatios.Count}, Item links: {_enchantItemGroups.Count}");
    }

    public GradeEnchantRatio GetGradeEnchantRatio(Item item)
    {
        if (item?.Template == null)
        {
            Logger.Warn("[Enchant] GetGradeEnchantRatio: item or template is null");
            return null;
        }

        int itemId = (int)item.Template.Id;

        // определяем группу
        if (!_enchantItemGroups.TryGetValue(itemId, out var groupId))
        {
            groupId = DefaultEnchantGroupId; // например 9000001
            Logger.Warn($"[Enchant] ItemId={itemId} has no group, fallback to {groupId}");
        }

        // проверяем наличие группы
        if (!_gradeEnchantRatios.TryGetValue(groupId, out var grades))
        {
            Logger.Warn($"[Enchant] GroupId={groupId} not found for ItemId={itemId}");
            return null;
        }

        // ищем строго по grade
        if (!grades.TryGetValue(item.Grade, out var ratio))
        {
            Logger.Warn($"[Enchant] No ratio for ItemId={itemId}, GroupId={groupId}, Grade={item.Grade}");
            return null;
        }

        Logger.Info(
            $"[Enchant] ItemId={itemId}, GroupId={groupId}, Grade={item.Grade}, " +
            $"S={ratio.SuccessRatio}, GS={ratio.GreatSuccessRatio}, B={ratio.BreakRatio}, D={ratio.DowngradeRatio}"
        );
