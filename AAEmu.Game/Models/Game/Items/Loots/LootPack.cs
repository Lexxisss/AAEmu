...
#pragma warning disable CA1859
    ...
    {
        if (killer is Character c)
            return c;

        if (killer is Units.Mate { OwnerObjId: > 0 } mate &&
            WorldManager.Instance.GetBaseUnit(mate.OwnerObjId) is Character mateOwner)
            return mateOwner;

        if (killer is Units.Slave
            { OwnerType: BaseUnitType.Character } slave &&
            WorldManager.Instance.GetBaseUnit(slave.OwnerObjId) is Character slaveOwner)
            return slaveOwner;

        if (killer is Doodad { OwnerType: DoodadOwnerType.Character } doodad &&
            WorldManager.Instance.GetBaseUnit(doodad.OwnerObjId) is Character doodadOwner)
            return doodadOwner;
